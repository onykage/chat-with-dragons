...
export async function GET(_: NextRequest, { params }: { params: Promise<{ refId: string }> }) {
  ...
  if (parts.length >= 3) {
    // Parse from the end to handle guild names with hyphens
    // Last part: randomId (l8jlsl)
    // Second to last: timestamp (1754872163453)
    // Third to last: level (L1)
    // Fourth to last: dungeonNumber (123)
    // Everything before that: guild (test-guild)
    ...
    if (parts.length >= 4) {
      ...
      for (let i = parts.length - 3; i >= 0; i--) {
        ...
        if (part.startsWith("L") && !isNaN(Number(part.substring(1)))) {
          ...
          // Assume this might be a level if it's a small number
          levelIndex = i
          level = Number(part)
          break
        }
      ...
      if (dungeonNumberIndex > 0) {
        ...
        // Fallback: assume first part is guild
        guild = parts[0]
        // Try to find any number in the remaining parts
    ...
    if (guild && dungeonNumber > 0) {
      try {
        ...
        // Generate a mock dungeon based on the parsed info
        ...
        console.log("Generated mock dungeon successfully:", {
          refId: mockDungeon.refId,
          guild: mockDungeon.guild,
          dungeonNumber: mockDungeon.dungeonNumber,
          level: mockDungeon.level,
          tilesCount: mockDungeon.tiles.length,
        })
      ...
      console.log("Could not parse guild and dungeon number from refId:", { guild, dungeonNumber })
      return new Response(
        `Could not parse refId: ${refId}. Expected format: guild-number-level-timestamp-id. Parsed: guild="${guild}", dungeonNumber=${dungeonNumber}`,
        { status: 400 },
      )
    }
  } else {
    console.log("RefId does not have enough parts for parsing:", parts.length)
    return new Response(`Invalid refId format: ${refId}. Expected format: guild-number-level-timestamp-id`, {
      status: 400,
    })
  }
}
...
function generateMockDungeonTiles(
  ...
  console.log("Generating tiles for dungeon:", { width, height, seed })
